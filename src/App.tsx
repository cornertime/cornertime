...
class App extends React.Component<{}, State> {
  ...
  state: State = {
    presetSource: JSON.stringify(defaultPreset, null, 2),
  };
            ...
            <div>
              <form>
                <label>
                  ...
                  <textarea rows={48} cols={90} value={this.state.presetSource} onChange={this.onChange} />
                </label>
              </form>
  ...
  onChange = (event: React.FormEvent<HTMLTextAreaElement>) => {
    this.setState({ presetSource: event.currentTarget.value });
  }
  ...
  loadPreset() {
    try {
      return deserializePreset(this.state.presetSource);
    } catch (e) {
      return JSON.parse(this.state.presetSource);
    }
  }
